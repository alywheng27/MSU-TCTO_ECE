...
<head>
    ...
    <title>Newsfeed - MSU-TCTO S-ECE</title>
...
<body >
	<div id="wrapper" >
		...
		<div id="page-wrapper" style="width: 80%; margin-left: 10%; border-left: 2px solid #dddddd;" >
			<?php
				if(($role == 'Administrator' and $_GET['post'] == 'Administrator') or ($role == 'Student' and $_GET['post'] != 'Administrator') or ($role == 'Instructor' and $_GET['post'] != 'Administrator')){
					echo '<div class="well " style="border: 0px; background: white;"> 
						<form action="index.php?post='.$_GET['post'].'" method="POST">
							<div style="display: -webkit-box; -webkit-box-orient: horizontal;">';
								...
								<textarea name="post_text" class="form-control" placeholder="What\'s on your mind:" rows="3" style="border-radius: 1px; width: 88%;" required></textarea>
							</div><br>
							<button type="submit" name="post_text_submit" class="btn btn-info" style="padding-left: 20px; padding-right: 20px; margin-left: 12%;">Post</button>
							<a href="?post='.$_GET['post'].'&add_photo=true" class="btn btn-danger" style="padding-left: 8px; padding-right: 8px; float: right; margin-right: 2%;"><i class="glyphicon glyphicon-camera"></i> Photo</a>
						</form>
					</div>
			...
			<?php
				$query = "SELECT accounts.id_number, first_name, middle_name, last_name, profile_photo, post, post_id, date FROM posts JOIN accounts ON accounts.id_number = posts.id_number WHERE admin_subject = '".$_GET['post']."' AND role != 'Parent' ORDER BY post_id DESC";
				$query_run = mysql_query($query);
				...
				if(mysql_num_rows($query_run) > 0){
					...
					<div style="width: 85%; position: relative;">
						...
						if($query_row['id_number'] == $id_number and $role == 'Student'){
							echo '<a href="?post='.$_GET['post'].'&edit_post='.$query_row['post_id'].'&enter_edit=true" style="float: right; margin-right: 3%; color: dimgrey; font-size: 13px;"><i class="glyphicon glyphicon-pencil"></i> Edit </a>';
						}
...
".$query_row['post']."
				...
				<div>
					<div style="display: -webkit-box; -webkit-box-orient: horizontal;">';
						...
						echo '<div style="width: 90%;">';
							...
							$query1 = "SELECT comment, comment_id, id_number_comment, first_name, middle_name, last_name, profile_photo FROM comments JOIN accounts ON accounts.id_number = comments.id_number_comment WHERE admin_subject_comment = '".$_GET['post']."' AND post_id = '".$query_row['post_id']."' AND role != 'Parent'";
							...
							while($query_row1 = mysql_fetch_assoc($query_run1)){
								echo '<div class="row">
									...
									<div class="col-md-11">
										...
										if($query_row['id_number'] == $id_number and $role == 'Student'){
											echo '<a href="?post='.$_GET['post'].'&delete_comment='.$query_row1['comment_id'].'" style="float: right; margin-right: 3%; color: dimgrey; font-size: 12px; margin-top: 23px;"><i class="glyphicon glyphicon-trash"></i> Delete </a>';
											echo '<a href="?post='.$_GET['post'].'&post_id='.$query_row['post_id'].'&edit_comment='.$query_row1['comment_id'].'" style="float: right; margin-right: 3%; color: dimgrey; font-size: 12px; margin-top: 23px;"><i class="glyphicon glyphicon-pencil"></i> Edit </a>';
										}
				...
				$query = "SELECT id_number, first_name, middle_name, last_name, profile_photo FROM accounts WHERE id_number = '00-42716' ";
				$query_run = mysql_query($query);
...
echo "<pre style='font-family: verdana; font-size: 30px; background-color: transparent; padding-left: 0px; border: 0px; margin-top: 20px;'>
