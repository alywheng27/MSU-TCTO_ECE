...
<body >
	<div id="wrapper" >
		...
		<div id="page-wrapper" style="width: 80%; margin-left: 10%; border-left: 2px solid #dddddd;">
				...
				if(($role == 'Administrator' and $_GET['post'] == 'Administrator') or ($role == 'Student' and $_GET['post'] != 'Administrator') or ($role == 'Instructor' and $_GET['post'] != 'Administrator')){
					echo '<div class="well " style="border: 0px; background: white;"> 
						<form action="index.php?student=true&post='.$_GET['post'].'" method="POST">
							...
							<button type="submit" class="btn-info" style="padding-left: 8px; padding-right: 8px;">Post</button>
							<button type="submit" class="btn-danger" style="padding-left: 8px; padding-right: 8px; float: right;"><i class="glyphicon glyphicon-camera"></i> Photo</button>
						</form>
					</div>
			...
			<?php
				$query = "SELECT first_name, middle_name, last_name, profile_photo, post, post_id, date FROM posts JOIN accounts ON accounts.id_number = posts.id_number WHERE admin_subject = '".$_GET['post']."'";
				$query_run = mysql_query($query);
...
".$query_row['post']."
				...
				<div>
					<div style="display: -webkit-box; -webkit-box-orient: horizontal;">
						<div style="width: 40px; margin-right: 5px; margin-top: 25px;">
							<img src="data:image/jpeg;base64,'.base64_encode($query_row['profile_photo']).'" style="width: 100%;" />
						</div>
						<div style="width: 90%;">';

							$query1 = "SELECT comment, id_number_comment, first_name, middle_name, last_name, profile_photo FROM comments JOIN accounts ON accounts.id_number = comments.id_number_comment WHERE admin_subject_comment = '".$_GET['post']."' AND post_id = '".$query_row['post_id']."'";
							...
							<form action="index.php?student=true&post='.$_GET['post'].'&post_id='.$query_row['post_id'].'" method="post">
								<div class="row">
									<div class="col-md-10">
